        ...
        <nav className="hidden lg:flex items-center gap-4 text-xs font-medium py-2">
          {navLinks.slice(0, 5).map((link) => (
            <Link
              key={link.href}
              href={link.href}
              className={`transition-colors whitespace-nowrap py-1 ${pathname === link.href ? 'text-blue-400 border-b-2 border-blue-400 font-bold' : 'text-slate-300 hover:text-white'}`}
            >
              {link.label}
            </Link>
          ))}
